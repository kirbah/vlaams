...
export default function App() {
  ...
  const [studyMode, setStudyMode] = useState<'due_only' | 'practice_all'>('due_only');
  ...
  const startSession = useCallback(
    (mode: 'due_only' | 'practice_all' = 'due_only', limit = dailyLimit, offset = 0) => {
      setStudyMode(mode);
  ...
  const handleAnswer = (isCorrect: boolean) => {
    ...
    const { updatedDeck, updatedProgress } = processAnswer(currentCard, isCorrect, progress, queue);
  ...
  const handleResetToDefault = () => {
    ...
    setBatchOffset(0);
    const active = initDeck(DEFAULT_WORDS, currentProg, 'due_only', dailyLimit, 0);
    setQueue(active);
    setInitialDueCount(active.length);
    setSessionReviewedCount(0);
  };
  ...
  const masteredCount = Object.values(progress).filter(p => p.streak >= 4).length;
